    ...
    def get(self):
        ...
        try:
            sql = "select a.hi_house_id, a.hi_title, a.hi_price, a.hi_ctime, b.ai_name, a.hi_index_image_url " \
                  "from ih_house_info a inner join ih_area_info b " \
        ...
        if result:
            for l in result:
                house = {
                    ...
                    "area_name": l["ai_name"],
                    "img_url": constant.QINIU_URL_PREFIX + l["hi_index_image_url"] if l["hi_index_image_url"] else ""
                }
                houses.append(house)
...
class HouseInfoHandler(BaseHandler):
    ...
    def post(self):
        ...
        try:
            ...
            logging.error(e)
            try:
                self.db.execute("delete from ih_hoouse_info where hi_house_id=%s", house_id)
            except Exception as e:
                logging.error(e)
                return self.write(dict(errcode=RET.DBERR, errmsg="delete failed"))
            else:
    ...
    def get(self):
        ...
        data = {
            ...
            "user_avatar": constant.QINIU_URL_PREFIX + ret["up_avatar"] if ret.get("up_avatar") else ""
        }
        ...
        if ret:
            for image in ret:
                images.append(constant.QINIU_URL_PREFIX + image["hi_url"])
        data["images"] = images
        ...
        json_data = json.dumps(data)
        try:
            self.redis.setex("house_info_%s" % house_id, constant.REDIS_HOUSE_INFO_EXPIRES_SECONDES, json_data)
        except Exception as e:
            logging.error(e)
...
class HouseImageHandler(BaseHandler):
    ...
    def post(self):
        ...
        if not img_name:
            return self.write({"errcode": RET.THIRDERR, "errmsg": "qiuniu error"})
        try:
...
class HouseListRedisHandler(BaseHandler):
    ...
    def get(self):
        ...
        if 1 == page:
            sql += " limit %s" % (constant.HOUSE_LIST_PAGE_CAPACITY * constant.HOUSE_LIST_PAGE_CACHE_NUM)
        else:
            sql += " limit %s,%s" % ((page - 1) * constant.HOUSE_LIST_PAGE_CAPACITY,
                                     constant.HOUSE_LIST_PAGE_CAPACITY * constant.HOUSE_LIST_PAGE_CACHE_NUM)
        ...
        if ret:
            for l in ret:
                house = dict(
                    ...
                    avatar=constant.QINIU_URL_PREFIX + l["up_avatar"] if l.get("up_avatar") else "",
                    image_url=constant.QINIU_URL_PREFIX + l["hi_index_image_url"] if l.get(
        ...
        # 对与返回的多页面数据进行分页处理
        # 首先取出用户想要获取的page页的数据
        current_page_data = data[:constant.HOUSE_LIST_PAGE_CAPACITY]
        house_data = {}
        ...
        while 1:
            page_data = data[
                        i * constant.HOUSE_LIST_PAGE_CAPACITY: (i + 1) * constant.HOUSE_LIST_PAGE_CAPACITY]
            if not page_data:
                break
        ...
        try:
            ...
            self.redis.expire(redis_key, constant.REDIS_HOUSE_LIST_EXPIRES_SECONDS)
        except Exception as e:
            logging.error(e)
